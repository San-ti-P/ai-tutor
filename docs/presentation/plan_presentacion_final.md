...
| "¿El Evaluator no puede equivocarse?" | **Sí, por eso implementamos LLM-as-judge.** Es un segundo LLM independiente que re-evalúa el 30% de las correcciones. Funciona así: (1) el Evaluator primario corrige la respuesta y asigna un score 0-10 con justificación; (2) el nodo `validate_feedback` aplica un muestreo aleatorio del 30% (`judge_sample_rate = 0.30`) activando la bandera `judge_sample`; (3) si la bandera está activa, el nodo `llm_judge` hace una llamada LLM independiente con el MISMO contexto (pregunta, respuesta base, respuesta del estudiante, chunks RAG, evaluación primaria) pero sin ver el prompt del Evaluator primario — es una segunda opinión ciega; (4) el juez produce un `JudgeVerdict` con su propio score, si coincide con el primario, y una explicación de discrepancias; (5) si `|score_primario - score_juez| > 2.0`, se marca `requires_review = True` para revisión humana. El juez NUNCA reemplaza el score primario — solo audita. Esto es LLM-as-judge real: mismo input, distinto LLM call, veredicto independiente. |
